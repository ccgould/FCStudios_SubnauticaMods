...
namespace FCS_AlterraHub.Patch
{
    internal class EnumCacheManager<T> where T : Enum
    {
        ...
        private void ReadCacheFile(string savePathDir, Action<int, string> loadParsedEntry)
        {
            ...
            {
                QuickLogger.Error($"Caught exception while reading {savePathDir}{Environment.NewLine}{exception}");
            }
        }
        ...
        internal void SaveCache()
        {
            ...
            {
                QuickLogger.Error($"Caught exception while saving cache!{Environment.NewLine}{exception}");
            }
        }
