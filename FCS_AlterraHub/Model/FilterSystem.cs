...
namespace FCS_AlterraHub.Model
{
    ...
    public static class FilterList
    {
        ...
        public static Filter FindCategory(TechType techType)
        {
            var filterList = GetFilters();
            foreach (Filter filter in filterList)
            {
                if (filter.HasTechType(techType) && filter.IsCategory())
                {
                    return filter;
                }
            }
        ...
        public static Filter FindFilter(TechType techType)
        {
            var filterList = GetFilters();
            foreach (Filter filter in filterList)
            {
            ...
            return AddUnknownEntry(techType);
        }
        ...
        {
            var path = Path.Combine(Mod.GetAssetPath(), "filters.json");
            ...
            var typeRefPath = Path.Combine( Mod.GetAssetPath(), "type_reference.json");
