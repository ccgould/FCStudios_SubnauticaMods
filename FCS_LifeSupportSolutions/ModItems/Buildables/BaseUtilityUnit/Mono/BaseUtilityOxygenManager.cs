...
internal class BaseUtilityOxygenManager : MonoBehaviour
{
    ...
    private BaseUtilityUnitController _controller;
    ...
    private void Awake()
    {
        _controller = gameObject.GetComponent<BaseUtilityUnitController>();
    }

    private void Update()
    {
        if (_controller == null || !_controller.IsOperational()) return;
    ...
    {
        if (!_controller.IsConstructed) return false;
    ...
    {
        if (_controller.CachedHabitatManager.HasEnoughPower(_controller.GetPowerUsage()))
        {
    ...
    {
        if (_o2Level <= 0 || !_controller.IsConstructed) return;

        var o2Manager = Player.main.oxygenMgr;

        float playerO2Request;

        if (BaseUtilityUnitBuildable.IsRefillableOxygenTanksInstalled && Player.main.oxygenMgr.HasOxygenTank())
        {
