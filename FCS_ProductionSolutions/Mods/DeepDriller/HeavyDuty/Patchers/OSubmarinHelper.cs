...
namespace FCS_ProductionSolutions.Mods.DeepDriller.HeavyDuty.Patchers
{
    ...
    class OSubmarinHelper
    {
		...
		private static readonly FieldInfo _isRotationEnabled = typeof(Builder).GetField("rotationEnabled", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        private static readonly Dictionary<int, KeyValuePair<Vector3, float>> DrillPlacementPositions = new Dictionary<int, KeyValuePair<Vector3, float>>()
        {
        ...
        private static readonly Dictionary<int, KeyValuePair<Vector3, float>> PlatformPlacementPositions = new Dictionary<int, KeyValuePair<Vector3, float>>()
        {
		...
		private static void PositionObject(ref Vector3 position, ref Quaternion rotation, Transform currentObject, bool initialInverted = false,bool isPlatform = false)
		{
            ...
            if (isPlatform)
            {
                position = Translate(currentObject.position, currentObject.rotation, PlatformPlacementPositions[direction].Key);
                rotation.eulerAngles = new Vector3(newAngles.x, newAngles.y + PlatformPlacementPositions[direction].Value, newAngles.z);
            }
            else
            {

                position = Translate(currentObject.position, currentObject.rotation, DrillPlacementPositions[direction].Key);
                rotation.eulerAngles = new Vector3(newAngles.x, newAngles.y + DrillPlacementPositions[direction].Value, newAngles.z);
            }
        }
		
		private static bool PlaceObject(GameObject targetObj, Vector3 hitPoint, ref Vector3 position, ref Quaternion rotation,bool isPlatform)
		{
			if (targetObj != null)
			{
        ...
        public static bool SetPlaceOnSurface_Prefix( RaycastHit hit, ref Vector3 position, ref Quaternion rotation)
        {
			...
			if (ghostModel?.name != null && ghostModel.name.StartsWith(ModelName))
			{
                ...
                if (hit.collider?.gameObject?.name != null && (hit.collider.gameObject.name.StartsWith(DrillTargetObjectName)) || hit.collider.gameObject.name.StartsWith(PlatformTargetObjectName))
                {
                    QuickLogger.Debug("Is Drill", true);
					return PlaceObject(hit.collider.gameObject.transform?.parent?.parent?.gameObject, hit.point, ref position, ref rotation, hit.collider.gameObject.name.StartsWith(PlatformTargetObjectName));
                }
                else
                {
        ...
        public static bool UpdateAllowed_Postfix(ref bool __result)
        {
            ...
            if (ghostModel?.name != null && ghostModel.name.StartsWith(ModelName))
            {
                ...
                if (Physics.Raycast(aimTransform.position, aimTransform.forward, out RaycastHit hit, pmd, lm.value, QueryTriggerInteraction.Ignore))
                    if (hit.collider?.gameObject != null && (hit.collider.gameObject.name.StartsWith(DrillTargetObjectName) || hit.collider.gameObject.name.StartsWith(PlatformTargetObjectName)))
                    {
