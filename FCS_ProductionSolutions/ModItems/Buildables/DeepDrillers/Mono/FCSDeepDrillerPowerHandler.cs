...
internal class FCSDeepDrillerPowerHandler : FCSPowerManager, IFCSStorage, IPowerManager
{
    ...
    [SerializeField] private DeepDrillerController controller;
    ...
    private void Update()
    {
        if (controller == null || !controller.IsConstructed ||
    ...
    {
        if (!controller.IsOperational() || !controller.GetOreGenerator().GetIsDrilling()) return;
    ...
    private void AttemptToChargeBattery()
    {
        ...
        {
            controller.CachedHabitatManager.GetSubRoot().powerRelay.ConsumeEnergy(amount, out float amountConsumed2);
    ...
    {
        return 0;//controller.UpgradeManager.Upgrades.Where(upgrade => upgrade.IsEnabled).Sum(upgrade => upgrade.PowerUsage);
    }
    ...
    internal void Initialize(DeepDrillerController mono)
    {
        if (IsInitialized) return;
        controller = mono;

        var thermal = gameObject.GetComponent<FCSDeepDrillerThermalController>();
    ...
    {
        return $"Solar panel (sun: {Mathf.RoundToInt(GetRechargeScalar() * 100f)}% charge {Mathf.RoundToInt(_powerBank.SolarPanel.GetCharge())}/{Mathf.RoundToInt(Plugin.Configuration.DDSolarCapacity)})";
    }
    ...
    public override void TogglePowerState()
    {
        ...
        {
            //controller.DisplayHandler.TurnOnDisplay();
            controller.PowerManager.SetPowerState(FCSPowerStates.Powered);
        }
        else
        {
            //controller.DisplayHandler.TurnOffDisplay();
            controller.PowerManager.SetPowerState(FCSPowerStates.Tripped);
        }
    }
