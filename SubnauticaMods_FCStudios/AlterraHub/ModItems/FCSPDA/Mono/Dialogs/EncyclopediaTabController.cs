...
internal class EncyclopediaTabController : Page, uGUI_IListEntryManager
{
    ...
    public GameObject prefabEntry => ModPrefabService.GetPrefab("EncyclopediaEntry");
    ...
    internal void Initialize()
    {
        ...
        iconCollapse = ModPrefabService.GetIconByName("arrowUp", Main.MODNAME);
        iconExpand = ModPrefabService.GetIconByName("arrowDown", Main.MODNAME);
        listCanvas = encycList.FindChild("Viewport").FindChild("Content").GetComponent<RectTransform>();

        pathNodeSprites = new UISpriteData
        {
            hovered = ModPrefabService.GetIconByName("EncyclopediaEntryBackgroundHover", Main.MODNAME),
            normal = ModPrefabService.GetIconByName("EncyclopediaEntryBackgroundNormal", Main.MODNAME),
            selected = ModPrefabService.GetIconByName("EncyclopediaEntryBackgroundSelected", Main.MODNAME)
        };
        ...
        entryNodeSprites = new UISpriteData
        {
            ...
            normal = ModPrefabService.GetIconByName("EncyclopediaEntryBackgroundNormal", Main.MODNAME),
            selected = ModPrefabService.GetIconByName("EncyclopediaEntryBackgroundSelected", Main.MODNAME),
            selectedHovered = ModPrefabService.GetIconByName("EncyclopediaEntryBackgroundHover", Main.MODNAME),
        };
    }
    ...
    private void SetImage(Texture2D texture)
    {
        ...
        if (texture != null)
        {
            float num = texture.height / (float)texture.width;
    ...
    private uGUI_ListEntry GetEntry()
    {
        ...
        if (pool.Count == 0)
        {
            for (int i = 0; i < 4; i++)
            {
                uGUI_ListEntry = Instantiate(prefabEntry).GetComponent<uGUI_ListEntry>();
    ...
    {
        foreach (var encyclopediaEntry in EncyclopediaService.EncyclopediaEntries)
        {
