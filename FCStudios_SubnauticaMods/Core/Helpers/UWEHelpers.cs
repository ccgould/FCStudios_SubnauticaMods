...
public static class UWEHelpers
{
    ...
    public static bool ConsumeIngredientsFor(ItemsContainer container, TechType techType)
    {
        ...
        QuickLogger.Debug($"TechData: {techData.Count} | {Language.main.Get(techType)}", true);
        ...
        if (techData != null)
        {
            foreach (Ingredient ingredient in techData)
            {
    ...
    public static bool CheckIfAllIngredientsAreAvailable(ItemsContainer container,  ReadOnlyCollection<Ingredient>techData)
    {
        foreach (Ingredient ingredient in techData)
        {
