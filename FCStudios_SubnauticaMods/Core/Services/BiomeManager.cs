...
public static class BiomeManager
{
    ...
    public static string GetBiome(Transform trans)
    {
        return LargeWorld.main.GetBiome(trans.position);
    }

    public static string CalculateBiome(Transform tr)
    {
        if (LargeWorld.main)
        {
    ...
    public static List<TechType> FindBiomeLoot(Transform tr, string currentBiome)
    {
    ...
    private static void AdditionalLoot(ref List<TechType> defaultLoot, string currentBiome)
    {
         ...
         * 
        Plugin.Configuration.Convert();
#if DEBUG
            QuickLogger.Debug($"Addition Ore: {Main.Configuration.DDBiomeOresTechType.Count}");
#endif

        foreach (KeyValuePair<string, List<TechType>> valuePair in Plugin.Configuration.DDBiomeOresTechType)
        {
#if DEBUG
