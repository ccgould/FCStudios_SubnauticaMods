...
public class SaveLoadDataService : MonoBehaviour,IDataService
{
    ...
    private const string DB_NAME = "SaveData.db";
    ...
    public T Load<T>(string slotId, string modPackID) where T : IDBEntity, new()
    {

        using var dbConnection = new SQLiteConnection(Path.Combine(SaveUtils.GetCurrentSaveDataDir(), DB_NAME));
        dbConnection.CreateTable<T>();

        var data = dbConnection.Find<T>(slotId);

        if (data == null)
        {
            QuickLogger.Info($"No data found for slot {slotId} in mod {modPackID}. Returning new GameData instance.");
    ...
    public void Save<T>(T gameData, string modPackID) where T : IDBEntity, new()
    {
        using var dbConnection = new SQLiteConnection(Path.Combine(SaveUtils.GetCurrentSaveDataDir(), DB_NAME));
        dbConnection.CreateTable<T>();

        QuickLogger.Debug($"Saving data for ID: {gameData.Id} in mod pack: {modPackID}");

        if (!IdExists<T>(dbConnection,gameData.Id))
        {
    ...
    public bool IdExists<T>(SQLiteConnection db, string id) where T : new()
    {
        var result = db.Find<T>(id);
        return result != null;
    }
    ...
    { 
        ...
        string filePath = Path.Combine(path, "SaveData.db");
