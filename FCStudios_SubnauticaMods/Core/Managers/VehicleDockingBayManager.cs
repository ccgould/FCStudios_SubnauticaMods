...
namespace FCS_AlterraHub.Core.Managers;
public class VehicleDockingBayManager : MonoBehaviour
{
        ...
        foreach (var vehicle in localVehicles)
        {
            ...
            foreach (var vehicleContainer in vehicleContainers)
            {
                foreach (var item in vehicleContainer.ToList())
                {
                    ...
                    if (_manager.IsAllowedToAdd(item.item))
                    {
    ...
    private bool ValidateCheck(InventoryItem item)
    {
        if (!_manager.IsDockingFilterAddedWithType(item.item.GetTechType()))
            return _manager.IsAllowedToAdd(item.item);
        QuickLogger.ModMessage(LanguageService.BlackListFormat(Language.main.Get(item.item.GetTechType())));
        return false;
    }
    ...
    private void OnDockedVehicleChanged()
    {
        ...
        HabitatService.main.GlobalNotifyByID(null, "VehicleUpdate");
    }
    ...
    {
        HabitatService.main.GlobalNotifyByID(null, "VehicleModuleAdded");
    }

    private void ModulesOnRemoveItem(InventoryItem item)
    {
        HabitatService.main.GlobalNotifyByID(null, "VehicleModuleRemoved");
    }
    ...
    public void Initialize(HabitatManager manager)
    {
        _manager = manager;
    }
    ...
    public bool HasVehicles(bool verbose = false)
    {
        ...
        if (!value && verbose)
        {
            QuickLogger.Message(LanguageService.NoVehiclesDocked(), true);
        }
    ...
    public List<ItemsContainer> GetVehicleContainers(Vehicle vehicle)
    {
    ...
    public bool IsVehicleDocked(object currentVehicle)
    {
